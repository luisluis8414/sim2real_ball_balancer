...
Run inside Isaac Sim with ``models/usd/scene.usda`` open (Script Editor, or execute it through the
python_server). The new pose is saved into the scene file.
...
SERVOS_PY = _driver_path()
exec(open(SERVOS_PY).read())
...
async def set_home_pose():
    ...
    # re-arm the servos so their models start from the new authored pose
    exec(open(SERVOS_PY).read(), globals())
...
builtins._set_home_task = asyncio.ensure_future(set_home_pose())  # runs on Kit's loop; exec() allows no top-level await
