...
Needs Isaac Sim launched with its Python Server (docs/simulation.md); the script opens the scene,
re-arms the servo models and restarts the timeline itself. Every leg -- home, min, max, home -- is
written to both at the same moment: to the rig through ballbal (``Rig.move_to``, with its stall
detection) and to the simulated STS3215 models (``src/ballbal/simulation/isaac.py``) with the same
speed. Both are recorded; a table compares where each axis ended up and how long it took. Take
the ball off the plate first.
...
def isaac(code: str, timeout: float = 30.0):
    """Run code in Isaac Sim. Returns a value only when ``code`` is a single expression."""
    return SIM.run(code, timeout=timeout)


RECORD = """
import builtins, omni.physx
builtins._svr_log = []
def _svr_rec(dt):
    p = servos.positions()
...
def main() -> int:
    ...
    SIM.arm()  # fresh models from the current profile
    SIM.stop()
    SIM.play()
    isaac(RECORD)
